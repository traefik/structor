...
func ListBranches(debug bool, branchPrefix string) ([]string, error) {
	opts := func(g *gTypes.Cmd) {
		g.AddOptions(fmt.Sprintf("origin\\/%s*", branchPrefix))
	}

	branchesRaw, err := git.Branch(branch.Remotes, branch.List, opts, git.Debugger(debug))
	if err != nil {
